...
public class StructureTest {
    ...
    public void CheckPlaceStructure() {
        AssertThat(Structure.CheckPlaceStructure(new List<Tile> { new LandTile(new Tile(), TileType.Grass) }, 0))
            .IsTrue();
    }

    [Test]
    public void InCityCheck() {
        AssertThat(Structure.InCityCheck(new List<Tile> { mockutil.GetInCityTile(1, 1), mockutil.GetInCityTile(1, 2) },
            0)).IsTrue();
    }

    [Test]
    public void InCityCheck_50percent() {
        AssertThat(Structure.InCityCheck(
            new List<Tile> { mockutil.GetInCityTile(1, 1), mockutil.GetInOtherCityTile(1, 2) }, 0)).IsTrue();
    }
    ...
    public void InCityCheck_33percent() {
        AssertThat(Structure.InCityCheck(new List<Tile> {
            mockutil.GetInCityTile(1, 1),
            mockutil.GetInOtherCityTile(1, 2), mockutil.GetInOtherCityTile(1, 3)
        }, 0)).IsFalse();
    }
    ...
    public void CanBuildOnSpot_NotAble() {
        AssertThat(Structure.CanBuildOnSpot(new List<Tile> { new LandTile(1, 1) { Type = TileType.Mountain } }))
            .IsFalse();
    }
    ...
    public void Destroy() {
        ...
        Structure.Destroy(mockutil.AttackMock.Object, false);
        AssertThat(Structure.CurrentHealth).IsLesserThanOrEqualTo(0);
        AssertThat(Structure.IsDestroyed).IsTrue();
        AssertThat(t.Structure).IsNull();
        AssertThat(Structure.OnDestroyCalled).IsTrue();
        AssertThat(mockutil.Callbacks).HasInvoked(cb => cb.StructureDestroy(Structure, mockutil.AttackMock.Object))
            .Once();
    }
