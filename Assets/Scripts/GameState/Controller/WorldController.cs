...
namespace Andja.Controller {
    ...
    public class WorldController : MonoBehaviour {
        ...
        public Action<Unit, IAttack> cbWorldUnitDestroyed;
        ...
        public float FixedDeltaTime {
            get { return Time.fixedDeltaTime * TimeMultiplier; }
        }
        ...
        public void SetGeneratedWorld(World world, Dictionary<Tile, Structure> tileToStructure, Rect spawnRect,
            Vector2[] spawnPoints) {
            this.World = world;
        ...
        private void CreatePlayerStarts(Vector2[] spawnPoints) {
            ...
            for (int i = 0; i < PlayerController.Instance.Players.Count; i++) {
                ...
                if (loadout.Structures != null && player.IsHuman &&
                    Array.Exists(loadout.Structures, x => x is WarehouseStructure)) {
                    //here then place em
                    AIPlayer temp = new AIPlayer(player, true);
                    var placeWarehouse = temp.DecideIsland(true);
                    BuildController.Instance.BuildOnTile(placeWarehouse.Item2,
                        placeWarehouse.Item2.GetBuildingTiles(placeWarehouse.Item1),
        ...
        public void RegisterWorldUnitDestroyed(Action<Unit, IAttack> callbackfunc) {
            cbWorldUnitDestroyed += callbackfunc;
        }

        public void UnregisterWorldUnitDestroyed(Action<Unit, IAttack> callbackfunc) {
            cbWorldUnitDestroyed -= callbackfunc;
        }
