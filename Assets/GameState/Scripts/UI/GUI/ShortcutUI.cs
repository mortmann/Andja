...
public class ShortcutUI : MonoBehaviour {
	...
	public void SetDragAndDropBuild(GameObject go){
		...
		dragADropGO.GetComponent<BuildingBuildUI> ().Show (go.GetComponent<BuildingBuildUI> ().structure,false);
	...
	public void StopDragAndDropBuild(){ 
		if (shortCut != -1) {
			...
			g.GetComponent<BuildingBuildUI> ().Show (dragADropGO.GetComponent<BuildingBuildUI> ().structure, true);
	...
	public void OnClick(int number){
		BuildController.Instance.OnClick (shortcuts[number].GetComponentInChildren<BuildingBuildUI> ().structure.ID);
	}
}
